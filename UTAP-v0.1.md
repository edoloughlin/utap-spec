...
timestamp, and a random nonce) using a CFP-held secret key. In v0.1.1, the
canonical input MAY additionally include version-defined security binding
fields (e.g., `intended_payee_canonical`). The credential is the value
embedded in URIs. It cannot be forged without the CFP's secret, and any
modification to the underlying attributes invalidates it.
...
**Intended Payee Binding (`intended_payee`)**
: An optional token field that binds a token to a specific Providing Agent
AID, or to a constrained wildcard AID pattern. When present, the CFP MUST
reject claim/hold/transfer attempts by authenticated PAs that do not match.
...
| `POST /api/claim`           | Atomic Claim / Presentation (8.4.1) |
| `POST /api/mass/ack`        | Acknowledgment & Burn (8.7)    |
...
JWT `iat`, `nbf`, and `exp` validation MUST apply bounded clock leeway
(RECOMMENDED: ±30 seconds). CFP nodes MUST use authenticated NTP (or
equivalent secure time synchronization), and inter-node clock skew MUST NOT
exceed 2 seconds. Security decisions involving expiry or ordering MUST use CFP
authoritative server time; client clocks MUST NOT be trusted.
...
```json
{
  ...
  "intended_payee": "utap:agent:cloudco.com:billing-agent",
  "status": "MINTED",
  "purpose": {
    "category": "compute",
...
| `version`               | string | MUST | Protocol version (`utap-0.1.1`)                      |
...
| `intended_payee`        | string | MAY  | Bound PA AID or constrained wildcard AID pattern     |
...
```
credential_input = token_id || owner_aid || amount || currency || timestamp || nonce || intended_payee_canonical
...
- `intended_payee_canonical`: Canonical payee-binding value:
  - `""` (empty string) if `intended_payee` is absent
  - exact `intended_payee` string if present
...
def mint_credential(cfp_secret, token_id, owner, amount, currency, timestamp, intended_payee=None):
    nonce = os.urandom(16)
    intended_payee_canonical = intended_payee or ""
    parts = [token_id, owner, amount, currency, timestamp, nonce.hex(), intended_payee_canonical]
...
```javascript
async function mintCredential(cfpSecret, tokenId, owner, amount, currency, timestamp, intendedPayee = null) {
  const nonce = crypto.getRandomValues(new Uint8Array(16));
  const nonceHex = Array.from(nonce).map(b => b.toString(16).padStart(2, "0")).join("");
  const intendedPayeeCanonical = intendedPayee ?? "";
  const parts = [tokenId, owner, amount, currency, timestamp, nonceHex, intendedPayeeCanonical];
...
```python
def verify_credential(cfp_secret, credential, token_id, owner, amount, currency, timestamp, nonce, intended_payee=None):
    intended_payee_canonical = intended_payee or ""
    parts = [token_id, owner, amount, currency, timestamp, nonce, intended_payee_canonical]
    credential_input = b"\x00".join(p.encode("utf-8") for p in parts)
...
#### 6.4.5 Versioned Credential Canonicalization

Because `intended_payee` is included in canonical input in v0.1.1, verifiers
MUST branch credential recomputation by token `version`:

- `utap-0.1`: legacy canonical input without `intended_payee`
- `utap-0.1.1`: canonical input with `intended_payee_canonical`

Mixed-version deployments SHOULD default to STRICT profile behavior for newly
minted tokens and support legacy verification only during migration windows.
Legacy tokens without `intended_payee` remain valid under COMPAT profile until
migration cutoff.
...
### 6.8 Recipient Binding (`intended_payee`)

The optional `intended_payee` field binds a token to a specific PA identity or
constrained PA set.

- If present, the CFP MUST enforce `intended_payee` on claim, hold, and
  transfer operations.
- If the authenticated PA does not match, CFP MUST reject with HTTP `403`
  and UTAP error code `PAYEE_MISMATCH`.
- The PA identity used for matching MUST come from the CFP-authenticated JWT
  context (`sub`), not from request-body fields.
- If `intended_payee` is absent, the token is transferable to any
  policy-permitted PA.

Implementations SHOULD support both:

1. Exact AID match: `utap:agent:cloudco.com:billing-agent`
2. Terminal wildcard local-id pattern: `utap:agent:cloudco.com:*`
...
```
https://{cfp-host}/pay?utap_version=0.1.1&utap_amount={amount}&utap_currency={currency}&utap_purpose={category}&utap_callback={callback_url}&utap_payee={payee_aid}
```

**Payment URI** (RA to PA -- "Here is my payment"):

```
https://{cfp-host}/pay?utap_credential={token_credential}&utap_version=0.1.1
```
...
| `utap_version`     | MUST     | Protocol version (e.g., `0.1.1`)                 |
...
| `utap_payee`       | MAY      | Expected PA AID hint for `intended_payee` binding |

When present, `utap_payee` is a hint to aid RA minting policy. Security
enforcement MUST rely on CFP-side JWT-authenticated PA identity matching
(Section 6.8), not on unauthenticated query parameters alone.
...
```
https://cfp.example.com/pay?utap_credential=NEW&utap_version=0.1.1&utap_amount=1500.00&utap_currency=USD&utap_purpose=compute&utap_callback=https://provider.example.com/webhook/utap&utap_payee=utap:agent:cloudco.com:billing-agent
```
...
The RA extracts the amount, currency, and purpose from the URI. If
`utap_payee` is present (or if the PA identity is known from an authenticated
channel), the RA SHOULD mint with `intended_payee` set accordingly. It then
mints a token via the CFP (see Section 8.2), receives the token credential in
the response, and replaces the request parameters:

```
https://cfp.example.com/pay?utap_credential=dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk&utap_version=0.1.1
```
...
- Additional mitigations against interception: short TTL (default 1 hour),
  single-use (burned after transfer), HTTPS transport, recipient binding
  (`intended_payee`), and the requirement that only an authenticated PA can
  claim/transfer.
- For HTTP integrations, credential placement in headers or JSON bodies SHOULD
  be preferred over query strings where feasible.
- If query strings are used, deployments MUST enforce redaction of
  `utap_credential` in access logs, reverse proxies, WAF logs, analytics
  pipelines, traces, crash reports, and telemetry labels.
- Deployments using browser-mediated flows MUST enforce referrer policy
  controls that prevent outbound leakage of query parameters.
- URIs SHOULD NOT be logged in plaintext by intermediary systems. Agents and
  intermediaries MUST treat UTAP credentials with the same confidentiality
  posture as bearer access tokens.
...
8. If `intended_payee` is present, it MUST match AID format from Section 5
   (or an allowed constrained wildcard form).
...
The CFP determines the presenting PA identity from the authenticated JWT
subject (`sub`), not from request-body fields.
...
### 8.4.1 Atomic Claim (STRICT Profile Default)

To eliminate validate→hold race windows, CFPs in STRICT profile MUST support:

`POST /cfp/v1/tokens/claim`

This endpoint performs validation and hold atomically in one serializable
transaction.

**Request:**

```http
POST /cfp/v1/tokens/claim HTTP/1.1
Host: cfp.example.com
Authorization: Bearer <pa-jwt>
Content-Type: application/json
Idempotency-Key: billing-agent-1740000250-claim-8821
...
**Atomic behavior (CFP MUST perform in one transaction):**

1. Credential verification.
2. Token state checks.
3. Policy checks, including `intended_payee` against authenticated PA.
4. Transition to `HELD` bound to presenting PA.
5. Hold expiry assignment.
6. Audit record write.

Concurrent claim attempts for the same token MUST result in exactly one
success. Race losers MUST receive `CLAIM_CONFLICT` (or implementation-equivalent
`TOKEN_STATE_CONFLICT`). Malformed claim payloads MUST return
`INVALID_CLAIM_REQUEST`; expired holds MUST return `HOLD_EXPIRED`.
...
### 8.5 Hold (Optional Escrow; COMPAT Profile)
...
resource delivery. In STRICT profile, implementations SHOULD prefer
`POST /cfp/v1/tokens/claim` (Section 8.4.1) over this two-step path.
...
If `intended_payee` is present, CFP MUST enforce it using the authenticated
JWT subject and reject mismatches with `PAYEE_MISMATCH`.
...
or serializable transactions) to prevent race conditions. If `intended_payee`
is present, the CFP MUST enforce it against the authenticated PA JWT subject
and reject mismatch attempts with `PAYEE_MISMATCH`.
...
The key MUST be unique per logical operation within its scoped uniqueness key.

Idempotency uniqueness MUST be keyed by:

`(agent_id, endpoint, idempotency_key, canonical_request_hash)`

`canonical_request_hash` is the hash of a deterministic canonical form of the
request body (and relevant query/path inputs for that endpoint).

The same key string reused by different agents MUST NOT collide.
...
| Scenario | CFP Action |
|----------|------------|
| New `(agent_id, endpoint, idempotency_key, canonical_request_hash)` tuple | Process normally. Store response with 24-hour TTL (or deployment policy). |
| Same tuple replay after success | Return cached result. Add `X-Idempotent-Replay: true` header. |
| Same `(agent_id, endpoint, idempotency_key)` but different payload hash | Reject with `400 INVALID_IDEMPOTENCY`. |
| Same tuple still processing | Return `409 IDEMPOTENCY_CONFLICT` with `Retry-After` header. |
| Prior tuple failed | Allow retry according to endpoint policy. |

### 11.4 Distinguishing Retries from Double-Spends

The interaction between scoped idempotency tuples and token IDs provides
double-spend detection:

| Same Scoped Tuple     | Same Token | Interpretation       | CFP Action |
|-----------------------|------------|----------------------|------------|
| Yes                   | Yes        | Retry (safe)         | Return cached result |
| No                    | Yes        | Double-spend attempt | Reject with 409    |
| Same key, hash differs | Any    | Client misuse / mismatch| Reject with 400 `INVALID_IDEMPOTENCY` |
...
All UTAP agent-to-CFP communications MUST use TLS 1.2 or later. The CFP MUST
NOT accept plaintext HTTP connections. Agents MUST validate TLS certificates
and MUST NOT disable certificate verification.

For agent-to-agent credential exchange, production deployments MUST use
encrypted transport. Non-confidential transport is permitted only in explicit
`HIGH_RISK_PROFILE` mode with compensating controls (short TTL, immediate
claim, replay monitoring, operator acknowledgement).
...
token's core attributes (ID, owner, amount, currency, timestamp, nonce)
using a secret key held only by the CFP. In v0.1.1, canonical input also
includes `intended_payee_canonical` when present. An attacker who intercepts
a credential cannot:
...
- **Recipient binding (`intended_payee`):** When present, only a matching
  authenticated PA may claim/hold/transfer the token.
- **Atomic claim:** `POST /cfp/v1/tokens/claim` removes validate→hold races by
  combining verification and hold in one transaction.
- **Transfer authorization:** The CFP requires PA authentication before
  executing transfer. Possessing a credential alone is not sufficient to
...
- **Token draining:** The compromised PA can accept any credential presented to
  it and immediately call `POST /cfp/v1/tokens/claim` (or legacy
  validate+transfer flow), redirecting funds to an account it controls.
...
2. **Atomic state transitions:** The CFP MUST use serialized access for token
   state changes. Two concurrent claim/transfer attempts for the same token
   MUST result in exactly one success and one conflict outcome
   (`CLAIM_CONFLICT`, `TOKEN_STATE_CONFLICT`, or `TOKEN_ALREADY_CLAIMED`).
...
The CFP MUST implement rate limiting to prevent abuse and credential
enumeration.

At minimum, `/cfp/v1/tokens/validate` and `/cfp/v1/tokens/claim` MUST be
limited using:

- Per-agent limits.
- Per-source-IP and/or subnet limits.
- Burst and sustained windows.

Minting and batch operations MUST also be rate-limited per deployment policy.
On repeated failures, CFP SHOULD apply progressive backoff and emit security
alerts.
...
### 12.9 Time Authority and Clock Drift

CFP nodes MUST synchronize using authenticated NTP (or equivalent secure time
source). Inter-node skew MUST NOT exceed 2 seconds.

- Health checks MUST alarm on skew policy violation.
- Deployments MAY fail closed when skew exceeds policy.
- Token expiry (`expires_at`) and security-critical ordering MUST use CFP
  authoritative server time.
- Business timestamps MAY remain second-level ISO 8601; security checks SHOULD
  use monotonic/authoritative server clocks.

### 12.10 Validation Oracle Hardening

Credential validation and claim endpoints SHOULD minimize distinguishability
between malformed, unknown, and non-transferable credential failures.

Where feasible, implementations SHOULD return a uniform error shape and similar
latency class to reduce enumeration signal.

### 12.11 Security Profile Matrix

| Profile             | Recipient Binding | Atomic Claim | A2A Encrypted Transport | Notes |
|---------------------|-------------------|--------------|--------------------------|-------|
| STRICT (recommended)| Required          | Required     | Required                 | Default for production; token TTL <=10 min (1-5 min preferred for high value) |
| COMPAT              | Optional          | Recommended  | Required                 | Migration profile; publish deprecation timeline |
| HIGH_RISK_PROFILE   | Required          | Required     | Not required             | Requires <=120s TTL, immediate claim, elevated monitoring |

### 12.12 Optional Proof-of-Possession Claim Profile

Deployments MAY enable `POP_CLAIM` as an additional protection:

- PA includes a signed nonce in `POST /cfp/v1/tokens/claim`.
- CFP verifies signature against PA registered public key.
- CFP enforces nonce freshness / one-time use.

This profile reduces usefulness of stolen bearer credentials and is OPTIONAL in
v0.1.1.
...
| 409  | `CLAIM_CONFLICT`        | Yes   | Concurrent claim in progress or just completed   |
...
Implementations MAY map `CLAIM_CONFLICT` to `TOKEN_STATE_CONFLICT` where
...
payment unit in UTAP. The credential provides tamper integrity, but it is still
a bearer secret and therefore requires confidentiality controls.

- **Agent-to-CFP communication** (minting, validation, claim, transfer, burn)
  MUST use authenticated, encrypted channels (HTTPS).
- **Agent-to-agent credential exchange** in production MUST use encrypted
  transport.
- Non-confidential channels are permitted only under explicit
  `HIGH_RISK_PROFILE` configuration (Section 14.5.7) with mandatory short TTL,
  immediate claim, replay monitoring, and operator acknowledgement.

All systems handling credentials MUST redact `utap_credential` from logs,
traces, metrics labels, crash reports, and analytics sinks.
...
| POST   | `/cfp/v1/tokens/claim`            | Yes  | Atomically validate + hold token |
| POST   | `/cfp/v1/tokens/{id}/hold`        | Yes  | Place hold on token (COMPAT) |
...
The following transports can carry UTAP credentials between agents. The
credential is the base64url-encoded HMAC string (43 characters). In all cases,
agent-to-CFP communication (validation, claim, transfer, burn) continues to use
the HTTP/JSON binding defined in Section 14.2.

Production deployments MUST use encrypted transport for agent-to-agent
credential exchange unless explicitly operating in `HIGH_RISK_PROFILE`.
`HIGH_RISK_PROFILE` requirements are defined in Section 14.5.7.
...
```json
{
  ...
  "utap_payee": "utap:agent:cloudco.com:billing-agent"
}
```
...
- The message queue MUST provide encryption in transit for production use.
  Unencrypted queue transport is allowed only under `HIGH_RISK_PROFILE`
  controls (Section 14.5.7).
...
- gRPC channels between agents MUST use TLS in production.
...
- MQTT brokers MUST require TLS (port 8883) in production.
...
QR-based credential exchange exposes bearer credentials visually and therefore
MUST be treated as `HIGH_RISK_PROFILE` unless an encrypted wrapper/channel is
used in addition to the QR payload.

- Displayed on a screen only for short-lived tokens.
- Printed media SHOULD be avoided for reusable operational flows.
- Recipients SHOULD immediately call `POST /cfp/v1/tokens/claim` upon receipt.
...
Raw NFC URI exchange SHOULD be paired with immediate claim semantics; if NFC
confidentiality guarantees are insufficient for the deployment context,
`HIGH_RISK_PROFILE` controls apply.
...
#### 14.5.7 SMS / Messaging (HIGH_RISK_PROFILE)

Credentials transmitted via SMS, email, or messaging APIs (Slack, WhatsApp
Business, etc.) are non-confidential by default and therefore allowed only in
explicitly configured `HIGH_RISK_PROFILE` deployments.

When `HIGH_RISK_PROFILE` is used, deployments MUST enforce all of the
following:

- Token TTL <= 120 seconds.
- Immediate `POST /cfp/v1/tokens/claim` on credential receipt.
- Replay anomaly detection and alerting enabled.
- Explicit operator acknowledgement of elevated transport risk.
- Credential redaction across logs, telemetry, and analytics sinks.

### 14.6 Transport Security Summary

| Transport              | Integrity   | Confidentiality | Allowed Profile(s)     |
|------------------------|-------------|-----------------|------------------------|
| HTTPS (URI)            | TLS + HMAC  | TLS             | STRICT, COMPAT         |
| Message Queue (TLS)    | HMAC        | TLS             | STRICT, COMPAT         |
| gRPC (TLS)             | HMAC        | TLS             | STRICT, COMPAT         |
| WebSocket (WSS)        | HMAC        | TLS             | STRICT, COMPAT         |
| MQTT (TLS)             | HMAC        | TLS             | STRICT, COMPAT         |
| QR Code (raw URI)      | HMAC        | None*           | HIGH_RISK_PROFILE only |
| NFC (raw URI)          | HMAC        | Proximity**     | HIGH_RISK_PROFILE***   |
| SMS / Messaging        | HMAC        | Varies          | HIGH_RISK_PROFILE only |

*QR codes are visually exposed and easy to capture.

**NFC confidentiality depends on physical assumptions and local RF threat
models.

***NFC with an encrypted higher-layer channel may be used in STRICT/COMPAT.

In all cases, credential redaction and replay monitoring controls MUST be
implemented.
...
```
https://cfp.example.com/pay?utap_credential=NEW&utap_version=0.1.1&utap_amount=1500.00&utap_currency=USD&utap_purpose=compute&utap_ref=gpu-quote-8821&utap_payee=utap:agent:cloudco.com:billing-agent
```
...
**Step 4: PA atomically claims the token.**
...
Content-Type: application/json
Idempotency-Key: ba-1740000250-claim-8821
...
**Total HTTP calls to CFP: 4** (mint, claim, transfer, burn).
...
The following purpose categories are defined for UTAP v0.1.1. Implementations
...
```json
{
  ...
  "$id": "https://utap.dev/schema/v0.1.1/token.json",
  "title": "UTAP Token",
  "type": "object",
  "required": [
  ...
  "properties": {
    ...
    "intended_payee": {
      "type": "string",
      "description": "Optional PA binding: exact AID or constrained wildcard",
      "pattern": "^utap:agent:.+:([^*]+|\\*)$"
    },
    "status": {
      "type": "string",
      "enum": ["PENDING_APPROVAL", "MINTED", "HELD", "TRANSFERRED", "BURNED", "EXPIRED", "REVOKED"]
    },
    "purpose": {
      "$ref": "#/$defs/purpose"
    },
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://utap.dev/schema/v0.1.1/audit-record.json",
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://utap.dev/schema/v0.1.1/error.json",
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://utap.dev/schema/v0.1.1/delegation.json",
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://utap.dev/schema/v0.1.1/claim-request.json",
  "title": "UTAP Claim Request",
  "type": "object",
  "required": ["credential", "expected_amount", "expected_purpose", "claim_ttl_seconds"],
  "properties": {
    "credential": {
      "type": "string"
    },
...
| `POST /api/claim` (ticket + pubkey)  | Atomic claim / payment validation     |
| `POST /api/mass/ack` (burn ticket)   | Token burn endpoint                   |
| `requireAuth` JWT middleware         | Agent authentication                  |
| `?ticket_id=<UUID>` in URL           | `?utap_credential=<HMAC>` in payment URI |
...
*End of UTAP Specification v0.1.1*
