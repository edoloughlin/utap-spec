...
an HMAC-SHA256 over the token's canonical core attributes (token ID, owner,
amount, timestamp, nonce, and version-defined security fields such as
`intended_payee_canonical`) using a CFP-held secret key. The credential is
the value embedded in URIs. It cannot be forged without the CFP's secret,
and any modification to the underlying attributes invalidates it.
...
- The `utap_credential` value in a URI is a **bearer credential**: any party
  that possesses the URI can initiate a validation request.
- Unlike a bare UUID, the credential is **tamper-proof**: an attacker who
  intercepts the URI cannot modify token attributes without invalidating the
  HMAC. They can only replay the credential as-is.
...
- Agents and intermediaries MUST treat UTAP credentials with the same
  confidentiality posture as bearer access tokens.
...
`intended_payee_canonical`) using a secret key held only by the CFP. An
attacker who intercepts a credential cannot:
